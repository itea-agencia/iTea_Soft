#!/usr/bin/env bash
# Rescata el asiento de regreso que quedo guardado en el tramo de vuelo, en produccion.
# Solo migra los tiquetes de un pasajero; lista los ambiguos sin tocarlos.
set -euo pipefail

DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ENV_FILE="$DIR/../.env.production"
SQL_FILE="$DIR/backfill_asiento_regreso_tiqueteria.sql"

...
| `limpiar_personas_huerfanas_create.{sh,sql}` | Borra 6 personas (id fijos: 66-71) que quedaron huerfanas por el bug viejo de `create` en `users.controller.js` (ya arreglado): alguien reintento el alta varias veces el 2026-07-22 y cada intento fallaba despues de crear la persona. Verificadas por nombre y por huerfandad antes de borrar; aborta si alguna cambio desde que se identificaron. | **aplicado en produccion el 2026-09-22** |
| `liberar_correos_de_eliminados.{sh,sql}` | Marca como eliminados a los usuarios que se "eliminaron" con el modelo viejo (inactivos con la persona eliminada) y libera su correo. No toca a los ambiguos: los lista para revisarlos a mano. Corre después de la migración `usuarios_deleted_at`. | **pendiente en producción** |
| `baja_de_admin.{sh,sql}` | Da de baja a un **administrador**, que la aplicación no deja desactivar ni eliminar. `./baja_de_admin.sh correo [eliminar]`. Aborta si el objetivo no es admin, ya no está activo, o si no quedaría otro admin activo. No limpia la caché del backend: reiniciarlo si la persona tuviera una sesión abierta. | herramienta, se corre cuando hace falta |
